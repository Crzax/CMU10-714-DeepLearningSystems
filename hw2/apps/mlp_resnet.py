...
def ResidualBlock(dim, hidden_dim, norm=nn.BatchNorm1d, drop_prob=0.1):
    ### BEGIN YOUR SOLUTION
    path = nn.Sequential(nn.Linear(dim,hidden_dim), norm(hidden_dim),nn.ReLU(),nn.Dropout(drop_prob),nn.Linear(hidden_dim,dim),norm(dim))
    ans = nn.Residual(path)
    return nn.Sequential(ans,nn.ReLU())
    ### END YOUR SOLUTION
...
def epoch(dataloader, model, opt=None):
    ...
    if opt is None:
      ...
      for X, y in dataloader:
        ...
        erro_all += np.sum(logits.numpy().argmax(axis=1) != y.numpy())
    else:
      model.train()
      for X, y in dataloader:
        logits = model(X)
    ...
    return erro_all / sample_nums, np.mean(loss_all)
    ### END YOUR SOLUTION
...
def train_mnist(
    ...
    for _ in range(epochs):
      train_erro, train_loss = epoch(train_loader, resnet, opt)
    test_err, test_loss = epoch(test_loader, resnet, None)
    return train_erro, train_loss, test_err, test_loss
    ### END YOUR SOLUTION
