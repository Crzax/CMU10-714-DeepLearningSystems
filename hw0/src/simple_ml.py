...
def parse_mnist(image_filename, label_filename):
    ...
    """
    ### BEGIN YOUR CODE
    with gzip.open(image_filename, 'rb') as imgs:
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(iter_nums):
      x = X[np.arange(i * batch, min((i+1)*batch, y.size)),:]
      y_iter = y[i*batch:min((i+1)*batch, y.size)]
      if (i == iter_nums - 1): batch = y.size - i * batch
      x_exp = np.exp(np.dot(x, theta))
      Z = x_exp / np.sum(x_exp, axis=1, keepdims=True)
      I_y = np.zeros((batch, Z.shape[1]))
      I_y[np.arange(batch), y_iter]=1
      gradient = np.dot(x.transpose(), (Z - I_y)) / batch
      theta -= lr * gradient
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
 ...
 s   Returns:
    ...
    iter_nums = (y.size + batch - 1) // batch
    for i in range(iter_nums):
      x = X[i*batch:min((i+1)*batch, y.size),:]
      y_iter = y[i*batch:min((i+1)*batch, y.size)]
      if (i == iter_nums - 1): batch = y.size - i * batch
      Z1 = x @ W1
      Z1[Z1 < 0] = 0
