...
class LayerNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ### BEGIN YOUR SOLUTION
        n, k = x.shape
        mean = (ops.summation(x,axes=(1,))/k).reshape((n,1)).broadcast_to((n,k))
        var = (((x-mean)**2).sum(axes=(1,))/k).reshape((n,1)).broadcast_to((n,k))
        deno = (var + self.eps)**0.5
        return self.w.reshape((1,k)).broadcast_to((n,k))*(x-mean)/deno + self.b.reshape((1,k)).broadcast_to((n,k))
        ### END YOUR SOLUTION
...
class Dropout(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if not self.training:
          return x
        msk = init.randb(*x.shape, p=1-self.p)
